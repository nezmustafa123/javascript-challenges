...
const interestedIn = prompt(
  "What do you want to know about Nez? Choose between firstName, lastName, age, job, and friends"
);
...
console.log(
  `${nezObject.firstName} has ${nezObject.friends.length} friends and his best friend is ${nezObject.friends[0]}`
);
...
const johnObject = {
  ...
  height: 1.95,
  calcBMI: function () {
    //inside the function create new property called bmi
    this.BMI = this.mass / this.height ** 2;
    return this.BMI;
  },
};
...
if (markObject.BMI > johnObject.BMI) {
  console.log(
    `Mark ${markObject.surName}'s BMI (${markObject.BMI}) is higher than John ${johnObject.surName}'s BMI (${johnObject.BMI})`
  );
} else if (johnObject.BMI > markObject.BMI) {
  console.log(
    `John ${johnObject.surName}'s BMI ${johnObject.BMI} is higher than Mark ${markObject.surName}'s BMI (${markObject.BMI})`
  );
}
